...
public class PhotoView extends ImageView {
    ...
    private final String TAG = PhotoView.class.getSimpleName();
    ...
    private void init() {
        ...
        //设置 mMinSpan ，防止双指距触摸距离过小不能缩小图片
        try {
            @SuppressLint("SoonBlockedPrivateApi") Field field = mScaleDetector.getClass().getDeclaredField("mMinSpan");
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            //左右滑动切换图片
            if (downX - e2.getX()  > 100) {
                EventBus.getDefault().post(new KeyCodeBean(1, KeyEvent.KEYCODE_MEDIA_NEXT));
            } else if (e2.getX() - downX > 100) {
                EventBus.getDefault().post(new KeyCodeBean(1, KeyEvent.KEYCODE_MEDIA_PREVIOUS));
            }
            isDown = true;
            Log.d(TAG, "onFling e2.getX " +  e2.getX() + " downX " + downX);
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            Log.d(TAG, "onScroll isDown " + isDown + " e2.getX " +  e2.getX() + " downX " + downX);
            if (isDown) {
                downX = e2.getX();
                downY = e2.getY();
                isDown = false;
